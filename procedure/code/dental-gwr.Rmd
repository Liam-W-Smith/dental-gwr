...
For the municipalities which have simply received a new ONS_code, I just the ONS codes in the extractions dataset to match the other two datasets.
```{r}
extractions <- extractions %>%
...
If two of my explanatory variables exhibit a correlation coefficient of over 0.6 or less than -0.6, I chose to include just one of them in my regression models. This helps prevent issues of multilinearity. Specifically, I remove crowded_rate and single_parent_households_ph. I'll perform a more rigorous test for multilinearity shortly.
...
We almost have enough information to make the maps imitating those shown in Broomhead et al's paper. The only thing I have to do before I can make those maps is determine which coefficients are statistically significant. Unfortunately, the authors did not describe how they determined statistical significance and there is no mentio of p-values. I had to do some online research in order to figure out a reasonable methodology, and I ended up applying the methodology presented in section 9.6.7 of this tutorial: https://gdsl-ul.github.io/san/gwr.html.
